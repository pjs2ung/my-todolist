...
| 2.4.0 | 2026-08-27 | 카테고리 등록/수정 화면 추가(계획 외 기능 확장, 사용자 요청 반영). 백엔드: `PATCH /categories/:id` 신규(이름 1~30자, unique 위반 시 `CATEGORY_NAME_TAKEN`, '기본' 카테고리는 이름 변경 시도 시 `DEFAULT_CATEGORY_RENAME_FORBIDDEN`으로 차단 — 자동적용/삭제방지 로직이 이름 '기본'으로 카테고리를 식별하기 때문), swagger.json/테스트 8건 추가(백엔드 전체 114건 통과). 프론트: `features/manage-categories`(생성/수정/삭제 mutation), `pages/category-page/CategoryPage.tsx`(인라인 수정, 네이티브 confirm 삭제), `/categories` 라우트, 할일목록 사이드바에 "카테고리 관리" 진입 링크 추가. Playwright로 생성/인라인수정/삭제취소·확정/필터 반영까지 실측 검증 | 사용자 요청("카테고리를 직접 만들고 수정하고 싶다") |
| 2.5.0 | 2026-08-27 | UI 조정(계획 외, 사용자 요청): "카테고리 관리" 버튼을 헤더로 옮겨 "새 할일 등록" 왼쪽에 배치 후 옅은 주황색 테마로 변경, 상태 필터 버튼이 선택 여부와 무관하게 항상 해당 상태 색상(뱃지와 동일)을 표시하도록 변경. 다크모드 토글 추가: `app/theme.css`(CSS 변수 기반 라이트/다크 토큰), `shared/lib/useTheme.ts`(localStorage 저장 + `data-theme` 속성 전환, 부팅 시 `initTheme()`으로 깜빡임 없이 복원), `shared/ui/ThemeToggle.tsx`(할일목록 사이드바 하단 배치). 전체 15개 CSS 파일의 배경/텍스트/테두리 색상을 변수로 전환, Playwright로 다크모드 전환+새로고침 유지+주요 화면(목록/등록폼/카테고리관리) 렌더링 실측 확인 | 사용자 요청(버튼 위치/필터 색상/다크모드) |
| 2.6.0 | 2026-08-27 | 다국어(i18n) 지원 추가(계획 외, 사용자 요청): `shared/lib/i18n.ts`(ko/en/ja 메시지 사전), `shared/lib/localeStore.ts`(zustand, localStorage 저장, `useT()` 훅), `shared/ui/LocaleToggle.tsx`(사이드바 다크모드 토글 아래 select box, 한국어/English/日本語). 전체 화면(인증/프로필/할일목록/등록폼/삭제다이얼로그/카테고리관리) 정적 텍스트를 `t.*` 키로 전환, 서버 검증 에러는 코드 기반 매핑(`registerFieldError`/`todoFormError`/`deleteTodoError`에 `locale` 파라미터 추가, 알려진 에러코드만 번역·분류 불가한 서버 원문 메시지는 기존 동작 유지)으로 3개 언어 번역. 카테고리/할일 제목 등 사용자 입력 데이터는 번역 대상에서 제외(의도된 범위). Playwright로 즉시 전환+새로고침 유지+다크모드 조합까지 실측 확인, 기존 단위테스트 37건 문구 변경 없이 그대로 통과 | 사용자 요청("다국어 처리해줘") |
| 2.7.0 | 2026-08-27 | 할일 제목 검색 추가(계획 외, 사용자 요청): `TodoListWidget`에 상태 필터와 같은 행, 우측 정렬 검색 input 추가. 백엔드 API 변경 없이 클라이언트에서 이미 조회된 목록을 제목 부분일치(대소문자 무시)로 필터링(`useMemo`), 3개 언어 placeholder 번역. Playwright로 일치/불일치 검색 결과 및 레이아웃 정렬 실측 확인, 기존 테스트 37건 통과 | 사용자 요청("할일 타이틀로 검색할 수 있는 input") |
| 2.8.0 | 2026-08-27 | 로그아웃 버튼 추가(계획 외, 사용자 요청): `features/logout`(`POST /auth/logout` 호출 후 성공/실패 무관하게 클라이언트 인증상태 정리, `/`로 이동), 사이드바 다국어 select box 바로 아래 배치. Playwright로 로그아웃 후 로그인화면 이동 및 `/todos` 직접 접근 시 재인증 안 됨(서버측 refresh 쿠키 무효화)까지 실측 확인, 기존 테스트 37건 통과 | 사용자 요청("로그아웃 버튼 만들어줘") |
| 2.9.0 | 2026-08-27 | 좌측 사이드바 고정(sticky) 처리(계획 외, 사용자 요청): `TodoListWidget.css`의 `.todo-list-sidebar`에 `position:sticky; top:24px; align-self:flex-start; max-height/overflow-y:auto` 적용(카테고리/필터가 많아지면 사이드바 자체는 내부 스크롤), 모바일(≤1024px) 브레이크포인트에서는 `position:static`으로 원복해 세로 스택 레이아웃 유지. DB에 할일 25건 삽입 후 Playwright로 800px/2500px 스크롤에도 사이드바 위치 고정 실측 확인, 테스트 데이터 정리, 기존 테스트 37건 통과 | 사용자 요청("좌측 메뉴가 스크롤에 움직이지 않아야") |
| 3.0.0 | 2026-08-27 | 상단 필터/검색 영역 고정 + 배경색 구분(계획 외, 사용자 요청): 제목/헤더 액션/상태필터/검색 input을 `.todo-list-sticky-toolbar`로 묶어 `position:sticky; top:24px` 적용(카드 목록 스크롤 시 상단에 고정), 사이드바는 `var(--color-surface)`, 툴바는 `var(--color-surface-alt)`, 카드 목록 영역은 기존 페이지 배경(`var(--color-bg)`)을 유지해 3개 구역이 서로 다른 색상으로 구분되도록 조정. Playwright로 할일 25건 스크롤 시 툴바+사이드바 동시 고정 및 배경색 구분 실측 확인, 기존 테스트 37건 통과 | 사용자 요청("상단 필터+검색 고정, 사이드바/상단 배경색 구분") |
| 3.1.0 | 2026-08-27 | 날짜 범위(from-to) 필터 추가(계획 외, 사용자 요청): 제목검색 입력 우측에 기존 `shared/ui/DatePicker` 재사용한 시작일/종료일 필터 추가(`~` 구분자), 백엔드 변경 없이 클라이언트에서 할일 기간과 선택 범위의 겹침 여부로 필터링(`todo.endDate >= from && todo.startDate <= to`, 한쪽만 입력해도 동작). 종료일 DatePicker의 `min`을 시작일에, 시작일의 `max`를 종료일에 연동해 역전 입력 1차 방지, 3개 언어 aria-label 추가. Playwright로 DB에 날짜가 다른 할일 2건 넣어 범위 필터링 결과 실측 확인, 기존 테스트 37건 통과 | 사용자 요청("제목검색 우측에 from-to 달력 필터") |
| 3.2.0 | 2026-08-27 | 필터 초기화 버튼 추가(계획 외, 사용자 요청): 카테고리/상태/제목검색/날짜범위 중 하나라도 적용되면 "초기화" 버튼이 상태필터("지연" 버튼) 우측에 나타나고, 클릭 시 `useTodoFilters`의 `resetFilters()`(categoryId/status 초기화)와 로컬 검색어/날짜 상태를 함께 초기화. 필터 미적용 시 버튼 숨김. Playwright로 필터 적용 전/후 버튼 노출 여부 및 초기화 동작 실측 확인, 기존 테스트 37건 통과 | 사용자 요청("필터 적용 시에만 초기화 버튼 노출, 지연 버튼 우측") |
| 3.3.0 | 2026-08-27 | 사이드바 스크롤 드리프트 버그 수정(2.9.0에서 남은 문제, 사용자 재현 리포트): 원인은 `.todo-list-sidebar`(flex item, align-self:stretch로 높이가 main만큼 늘어남)에 `position:sticky`를 직접 걸면 Chromium이 sticky를 아예 무시하던 것 — 실측(`getBoundingClientRect`)으로 확인. sticky를 stretch되는 flex item 자신이 아니라 그 안의 비-flex-item 자식(`.todo-list-sidebar-sticky`, 신규 래퍼)으로 한 단계 내려서 적용, 시각적 패널(`.todo-list-sidebar-inner`, 뷰포트 높이로 캡+내부 스크롤)은 그 안에 유지. Playwright로 scrollY 0~3500px 전 구간에서 `getBoundingClientRect().top`이 24px로 고정됨을 실측 확인(수정 전에는 scrollY와 1:1로 움직여 sticky가 전혀 동작하지 않았음), 기존 테스트 37건 통과 | 사용자 재현("스크롤 내리면 사이드바가 살짝 위로 이동") |
